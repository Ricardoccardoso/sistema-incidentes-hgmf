...
from db import (
    load_data_with_id, load_config, load_users,
    save_config_opcao, add_config_opcao,
    save_user, delete_user,
    update_status, insert_incidente,
    get_opcoes, hash_senha, COLUNAS_DADOS
)
...
[data-testid="metric-container"] [data-testid="stMetricValue"] {
    font-size: 2rem !important; font-weight: 700 !important; color: #0d47a1 !important;
}
[data-testid="metric-container"] [data-testid="stMetricLabel"] {
    font-size: 0.78rem !important; font-weight: 600 !important;
    text-transform: uppercase; letter-spacing: 0.8px; color: #546e7a !important;
}

.card-incidente {
    background: #fff; border: 1px solid #e8edf5;
    border-left: 4px solid #1976d2; border-radius: 10px;
...
.secao-titulo {
    ...
    margin: 20px 0 12px 0; padding-bottom: 6px; border-bottom: 2px solid #e3f2fd;
}
...
# ─── PAINEL AUTENTICADO ───────────────────────────────────────────────────────
# Carrega dados com ID para poder atualizar status individualmente
df_dados_raw = load_data_with_id()
# Versão sem a coluna id para uso nos cálculos
df_dados = df_dados_raw.drop(columns=["id"], errors="ignore")
...
if menu == "📊 Dashboard":
    ...
    if df_dados.empty:
        st.info("Nenhuma notificação registrada ainda. Assim que houver dados, os gráficos aparecerão aqui.")
    else:
        ...
        with g3:
            ...
            chart_sem = alt.Chart(df_sem).mark_line(point=True, strokeWidth=2, color="#1565c0").encode(
                x=alt.X("Semana:O"), y=alt.Y("Qtd:Q"), tooltip=["Semana","Qtd"]
        ...
        with g4:
            ...
            chart_set = alt.Chart(df_set).mark_bar(cornerRadiusTopRight=5, cornerRadiusTopLeft=5).encode(
                x=alt.X("Qtd:Q"), y=alt.Y("Setor:N", sort="-x"),
                color=alt.value("#0288d1"), tooltip=["Setor","Qtd"]
            ).properties(height=250)
        ...
        with g5:
            ...
            chart_turno = alt.Chart(df_turno).mark_bar().encode(
                x=alt.X("Turno:N"), y=alt.Y("Qtd:Q"),
                color=alt.Color("Turno:N", legend=None), tooltip=["Turno","Qtd"]
        ...
        with g6:
            ...
            if df_f["Fatores_Causadores"].notna().any():
                ...
                chart_fat = alt.Chart(df_fat).mark_bar(cornerRadiusTopRight=4).encode(
                    x=alt.X("Qtd:Q"), y=alt.Y("Fator:N", sort="-x"),
                    color=alt.value("#6a1b9a"), tooltip=["Fator","Qtd"]
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if df_dados.empty:
        ...
        # Aplica filtros ao df com ID
        df_view = df_dados_raw.copy()
        df_view["Data_Incidente"] = pd.to_datetime(df_view["Data_Incidente"], errors="coerce")
        if f_status != "Todos":
        ...
        for _, row in df_view.sort_values("Data_Registro", ascending=False).iterrows():
            ...
            with st.expander(f"Ver detalhes e gerenciar — Notificação #{seq}"):
                d1, d2 = st.columns(2)
                with d1:
                ...
                if perm in ["Acesso Total", "Apenas Relatórios"]:
                    ...
                    with col_s1:
                        novo_status = st.selectbox(
                            ...
                            key=f"status_{row_db_id}"
                        )
                    with col_s2:
                        if st.button("💾 Salvar Status", key=f"btn_status_{row_db_id}"):
                            ok = update_status(row_db_id, novo_status)
                            if ok:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if df_dados.empty:
        st.info("Sem dados para gerar relatórios. Aguardando primeiras notificações.")
    else:
        df_dados["Data_Incidente"] = pd.to_datetime(df_dados["Data_Incidente"], errors="coerce")
        ...
        if tipo_rel == "Resumo Mensal por Categoria":
            ...
            chart = alt.Chart(df_rank).mark_bar(cornerRadiusTopRight=5).encode(
                x=alt.X("Total:Q"), y=alt.Y("Setor:N", sort="-x"),
                color=alt.value("#1565c0"), tooltip=["Setor","Total","Graves","NearMiss"]
            ...
            if not df_lpp.empty:
                ...
                chart = alt.Chart(df_lpp_g).mark_bar().encode(
                    x="Qtd:Q", y=alt.Y("Estágio:N", sort="-x"),
                    color=alt.value("#c62828"), tooltip=["Estágio","Qtd"]
                ).properties(height=250)
                st.altair_chart(chart, use_container_width=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if df_dados.empty:
        st.info("Sem dados para exportar ainda.")
    else:
        ...
        df_exp = df_dados.copy()
        df_exp["Data_Incidente"] = pd.to_datetime(df_exp["Data_Incidente"], errors="coerce")
        df_exp = df_exp[
            (df_exp["Data_Incidente"] >= pd.to_datetime(data_ini_e)) &
            (df_exp["Data_Incidente"] <= pd.to_datetime(data_fim_e))
        ].copy()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if df_config.empty:
        st.warning("Sem configurações carregadas. Verifique a conexão com o banco de dados.")
    else:
        ...
        with col_t1:
            ...
            if st.button("💾 Gravar Alterações", type="primary"):
                for _, r in df_ed.iterrows():
                    save_config_opcao(tab_sel, r["Opcao"], bool(r["Ativo"]))
        ...
        with col_t2:
            ...
            if st.button("Adicionar", use_container_width=True):
                if n_op.strip():
                    ...
                    if not existe:
                        ok = add_config_opcao(tab_sel, n_op.strip())
                        if ok:
    ...
    with u1:
        ...
        with col_pc:
            if st.button("💾 Salvar Alterações", type="primary", use_container_width=True):
                updated = row_sel.to_dict()
                updated["Permissao"] = nova_perm
                updated["Ativo"]     = novo_ativo
                save_user(updated)
        ...
        if st.button("Redefinir Senha", use_container_width=True):
            if nova_senha_admin.strip():
                row_t = df_usuarios[df_usuarios["Usuario"] == usr_troca].iloc[0].to_dict()
                row_t["Senha_Hash"] = hash_senha(nova_senha_admin)
                save_user(row_t)
